...
#include "ui_LDbcSettingsWidget.h"
#include <QMessageBox>

LDbcSettingsWidget::LDbcSettingsWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::LDbcSettingsWidget)
{
    ui->setupUi(this);
...
LDbcSettingsWidget::~LDbcSettingsWidget()
{
    delete ui;
}

void LDbcSettingsWidget::setCanDataManager(LCanDataManager *a_pCanDataManager)
{
    m_pCanDataManager = a_pCanDataManager;
}

void LDbcSettingsWidget::on_buttonConnect_clicked()
{
...
void LDbcSettingsWidget::on_buttonDisconnect_clicked()
{
    if(m_pCanDataManager == nullptr) return;
...
void LDbcSettingsWidget::deviceConnected()
{
    unsigned long deviceIndex = ui->devIndexBox->currentText().toInt();
    unsigned long canIndex = ui->channelIndexBox->currentText().toInt();
    unsigned long deviceType = ui->devTypeBox->currentData().toInt();
